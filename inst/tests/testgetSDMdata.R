...
#     expect_that(length(dt) != 0, is_true()) # test we got some data
...
#                  stringsAsFactors = FALSE)
# dt3 <- getSDMdata(sp, dst, syr, eyr)
# test_that("getSDMdata multiple species", {
#     expect_that(("name" %in% colnames(dt3)), is_true()) ## we have more than one species, so names should be output
#     expect_that(("x" %in% colnames(dt3)), is_true())
#     expect_that(("y" %in% colnames(dt3)), is_true())
#     expect_that(is.unsorted(dt3$name), is_false()) ## should be sorted by species
# })

# implement this test when all data is available
# dt4 <- getSDMdata(sp, dst, syr, eyr, precision=0, units="m")
# test_that("getSDMdata precision and units", {
#     expect_that(nrow(dt4) > nrow(dt), is_true()) ## dt2 should include 10kms
#     expect_that(min(dt4$x) > 0, is_true())
#     expect_that(max(dt4$x) < 700, is_false()) ## should now be in m
#     expect_that(max(dt4$x) < 700000, is_true())
#     expect_that(min(dt4$y) > 0, is_true())
#     expect_that(max(dt4$y) < 1250, is_false()) ## should now be in m
#     expect_that(max(dt4$y) < 1250000, is_true())
#     
# })
#rm(dt, dt2, dt3, dt4)
